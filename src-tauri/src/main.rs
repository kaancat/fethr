...
async fn get_dashboard_stats(app_handle: AppHandle) -> Result<DashboardStats, String> {
    ...
    // For now, calculate streak based on history (simple version)
    // This could be improved later
    let daily_streak = 1; // Placeholder
    ...
    match stats_response {
        Ok(Ok(resp)) => {
            ...
            if status.is_success() {
                ...
                println!("[RUST CMD] Raw database response: {}", response_text);
...
// --- END Dashboard Stats with Auth Command ---
